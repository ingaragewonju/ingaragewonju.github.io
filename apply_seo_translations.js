...
const path = 'c:/Users/UserK/Desktop/초보프로젝트/app.js';
let appJs = fs.readFileSync(path, 'utf8');
...
for (const [lang, appendText] of Object.entries(translations)) {
    const target = \`catLocalAll: "\${lang === 'es' ? 'Todas las profesiones en tu país' : 
                      lang === 'zh' ? '国内所有职业' :
                      lang === 'ja' ? '国内のすべての職種' :
                      lang === 'hi' ? 'आपके देश में सभी पेशे' :
                      lang === 'ar' ? 'جميع المهن في بلدك' :
                      lang === 'fr' ? 'Toutes les professions dans votre pays' :
                      lang === 'ru' ? 'Все профессии в вашей стране' :
                      lang === 'pt' ? 'Todas as profissões no seu país' :
                      lang === 'de' ? 'Alle Berufe in Ihrem Land' : ''}", catGlobalAll: "\${lang === 'es' ? 'Todas las profesiones a nivel mundial' : 
                      lang === 'zh' ? '全球所有职业' :
                      lang === 'ja' ? '世界のすべての職種' :
                      lang === 'hi' ? 'दुनिया भर में सभी पेशे' :
                      lang === 'ar' ? 'جميع المهن حول العالم' :
                      lang === 'fr' ? 'Toutes les professions dans le monde' :
                      lang === 'ru' ? 'Все профессии в мире' :
                      lang === 'pt' ? 'Todas as profissões no mundo' :
                      lang === 'de' ? 'Alle Berufe weltweit' : ''}"\`;
    
    appJs = appJs.replace(target, appendText);
}

fs.writeFileSync(path, appJs);
